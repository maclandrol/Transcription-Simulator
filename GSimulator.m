function [liste, handle]= GSimulator(mode, time, trans, darn ,arnnumber,  trad, dprot, protnumber,ag , ig,div, celldiv )
...
clc;
if(isequal(div, 'No') || isequal(div, 'no') )
    if(isequal(mode, 'Burst'))
        %disp('Burst');
        [liste, handle]= burst(time, trans, darn ,arnnumber,  ag , trad, dprot, protnumber, ig );
    elseif(isequal(mode,'Continuous'))
        [liste, handle] = cont(time, trans, darn, arnnumber, trad, dprot, protnumber);
        %disp('Continuous');
    end
else
    [liste, handle]= CellDivModel(mode, time, trans, darn ,arnnumber,  trad, dprot, protnumber,ag , ig, celldiv);
end


end

function [liste, handle] = cont(time, trans, darn, arnnumber, trad, dprot, protnumber)
...
if(trad~=0 || dprot ~=0)
    ...
    mean_mRNA_theoretical = trans/darn; %steady-state mean mRNA
    noise_mRNA_theoretical = sqrt(1/mean_mRNA_theoretical); %steady-state noise mRNA
    mean_protein_theoretical = (trans*trad)/(darn*dprot); %steady-state mean protein
    noise_protein_theoretical = sqrt((1/mean_protein_theoretical)+ (1/(mean_mRNA_theoretical*(1+(darn/dprot))))); %steady-state noise protein
    
    fprintf('\n-----------------Analytical result-----------------\n\nmean(mRNA)= %.2f\nnoise(mRNA)= %.2f\nmean(Protein)= %.2f\nnoise(Protein)= %.2f\n',  mean_mRNA_theoretical , noise_mRNA_theoretical,mean_protein_theoretical,noise_protein_theoretical);
    ...
    save('data.mat', 't_array', 'prom_array', 'arn_array', 'prot_array', 'tau_array');
    ...
    mean_mRNA_theoretical = trans/darn; %steady-state mean mRNA
    noise_mRNA_theoretical = sqrt(1/mean_mRNA_theoretical); %steady-state noise mRNA
    fprintf('\n-----------------Analytical result-----------------\n\nmean(mRNA)= %.2f\nnoise(mRNA)= %.2f\n',  mean_mRNA_theoretical , noise_mRNA_theoretical);
    ...
    fprintf('\n-----------------Fitting poisson distribution-----------------\n');
    [H P STATS] = chi2gof(arn_array,'cdf', @(z)poisscdf(z,mean(arn_array)), 'nparams', 1, 'nbins', max(arn_array)+1-min(arn_array))
    save('data.mat', 't_array', 'prom_array', 'arn_array' , 'tau_array');
end
end


%%% Function burst
function [liste, handle]= burst(time, trans, darn ,arnnumber,  ag , trad, dprot, protnumber, ig )
...
if(trad==0 && dprot==0)
    ...
    fprintf('Simulation total time: %f seconds\n', exc_time);
    %% analytical results for the model  
    mean_mRNA_theoretical = (trans/darn)*(ag/(ag+ig)); %steady-state mean mRNA
    noise_mRNA_theoretical = sqrt(1/mean_mRNA_theoretical); %steady-state noise mRNA
    fprintf('\n-----------------Analytical result-----------------\n\nmean(mRNA)= %.2f\nnoise(mRNA)= %.2f\n',  mean_mRNA_theoretical , noise_mRNA_theoretical);
...
%     [H P STATS] = chi2gof(arn_array,'cdf', @(z)poisscdf(z,mean(arn_array)), 'nparams', 1, 'nbins', max(arn_array)+1-min(arn_array))
    ...
    mean_mRNA_theoretical = (trans/darn)*(ag/(ag+ig)); %steady-state mean mRNA
    noise_mRNA_theoretical = sqrt(1/mean_mRNA_theoretical); %steady-state noise mRNA
    mean_protein_theoretical = mean_mRNA_theoretical*(trad/dprot); %steady-state mean protein
    fprintf('\n-----------------Analytical result-----------------\n\nmean(mRNA)= %.2f\nnoise(mRNA)= %.2f\nmean(Protein)= %.2f\n',  mean_mRNA_theoretical , noise_mRNA_theoretical,mean_protein_theoretical);
